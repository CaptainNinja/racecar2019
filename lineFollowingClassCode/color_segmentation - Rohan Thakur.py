...
def cd_color_segmentation(img, HSV_vals, show_image=False):
    """
    ...
    new_img = cv2.cvtColor(img, cv2.COLOR_BGR2HSV)[img.shape[0] * 1/3:img.shape[0]]

    # define lower and upper bound of image values
    low_range  = np.array(HSV_vals[0])
    high_range = np.array(HSV_vals[1])
    ...
    # Return bounding box
    return ((x1, y1), (x1 + x2,  y1 + y2)), img, new_img
