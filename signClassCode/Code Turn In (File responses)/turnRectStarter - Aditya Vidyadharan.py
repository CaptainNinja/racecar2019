...
def sift_det(image, source):
    ...
    frame = source
    crop = frame
    if frame.all() != None:             # Did we get an image at all?
        return
    ...
    if des_s != None or len(matches) != 0:
        return
    ...
    if len(good) >= MIN_MATCH_COUNT:
        ...
        if M is not None:
            ...
            sign_box = ((intDist[0][0][0],intDist[0][0][1]),(intDist[2][0][0],intDist[2][0][1]))
    ...
    turnLeft = None
    if w != None and w != 0 and intDst[0][0][0] != None and intDst[2][0][0] != None and abs(intDst[0][0][0]-intDst[2][0][0]) != 0:
        if (intDst[0][0][0] + intDst[2][0][0])/2 > (2*x+w)/2:
            turnLeft = True
            print((intDist[0][0][0]+intDist[2][0][0])/2)
            print((2*x+w)/2)
        else:
            turnLeft = False
    return turnLeft
