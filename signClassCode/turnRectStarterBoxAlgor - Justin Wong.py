...
def sift_det(image, source):
    ...
    if len(contours) != 0:
        ...
        if w*h > 4000:
            greenBoxCoordinates.append((x,y))
            greenBoxCoordinates.append((x+w,y+h))
            if len(greenBoxCoordinates) > 20:
                greenBoxCoordinates = [(sum(greenBoxCoordinates[::2][0]) / len(greenBoxCoordinates) / 2.0), (sum(greenBoxCoordinates[1::2][1]) / len(greenBoxCoordinates) / 2.0)]
    ...
    if des_s is None and len(matches) == 0:
        print('des_s is None and len(matches) is 0')
    ...
    if len(good) >= MIN_MATCH_COUNT:
        ...
        if M is not None:
            ...
            area = (intDst[2][0][0] - intDst[0][0][0]) * (intDst[2][0][1] - intDst[0][0][1])
            redBoxCoordinates.append((intDst[0][0][0], intDst[0][0][1]))
            redBoxCoordinates.append((intDst[2][0][0], intDst[2][0][1]))
            if len(redBoxCoordinates) > 20:
                redBoxCoordinates = [(sum(redBoxCoordinates[::2][0]) / len(redBoxCoordinates) / 2.0), (sum(redBoxCoordinates[1::2][1]) / len(redBoxCoordinates) / 2.0)]
            if not (redBoxCoordinates[-2][0] < (greenBoxCoordinates[-2][0] + greenBoxCoordinates[-1][0]) / 2.0 < redBoxCoordinates[-1][0]) or not (redBoxCoordinates[-2][1] < (greenBoxCoordinates[-2][1] + greenBoxCoordinates[-1][1]) / 2.0 < redBoxCoordinates[-1][1]):
                greenBoxCoordinates = greenBoxCoordinates[:-2]
                redBoxCoordinates = redBoxCoordinates[:-2]
    ...
    try:
        ...
        print('there was a cv2.error')

    return (greenBoxCoordinates, redBoxCoordinates, area, frame)
